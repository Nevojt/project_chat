...
@pytest.fixture
def test_user():
    return {"email": "test_user@gmail.com", "password": "password123"}

@pytest.fixture
def test_user_update():
    return {"user_name": "User test Update", "avatar": "New avatar"}
...
def test_incorrect_login(email, password, status_code):
...
async def test_verify_user(test_user):
    async with AsyncClient(app=app, base_url="http://test") as client:
        # Create user before verification
        ...
        response = await client.get(f"/success_registration", headers=headers, params={'token': token_verify})
...
async def test_update_user(test_user, test_user_update):
    async with AsyncClient(app=app, base_url="http://test") as client:
        ...
        # Update the user
        new_data = {"user_name": test_user_update["user_name"], "avatar": test_user_update["avatar"]}
        headers = {"Authorization": f"Bearer {token}"}
        response = await client.put(f"/users/", headers=headers, json=new_data)
        assert response.status_code == 200
        assert response.json()['user_name'] == test_user_update["user_name"]

@pytest.fixture
def test_user_new_password():
    return {"email": "test_user@gmail.com", "password": "NewPassword"}
...
async def test_change_user_password(test_user, test_user_new_password):
    async with AsyncClient(app=app, base_url="http://test") as client:
        ...
        headers = {"Authorization": f"Bearer {token}"}
        change_password = {"old_password": test_user['password'], "new_password": test_user_new_password['password']}
        res_password = await client.put("/manipulation/password", headers=headers, json=change_password)
...
async def test_delete_user(test_user_new_password):
    async with AsyncClient(app=app, base_url="http://test") as client:
        ...
        headers = {"Authorization": f"Bearer {token}"}
        data = {"password": test_user_new_password['password']}
